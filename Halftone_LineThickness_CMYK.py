# -*- coding: utf-8 -*-
"""
Created on Sun Nov 27 21:33:51 2022
...
ImCMYK,x,y,X,Y=fu.readimage_normalized(Filename,Contrast,ResizedPX)
ImCMYK=fu.readimageCMYK(Filename,Contrast,ResizedPX)
...
if SaveImage is True:
    fu.SaveImagec(Name + '_HaftoneLiethicknessCMYK')
plt.show()


Names=['GnBu','Oranges','cividis','binary']
Names=['binary','binary','binary','binary']
Name2=['C','M','Y','K']

for lauf in range(4):
    plt.imshow(1-Im[:,:,lauf],cmap=Names[lauf],origin='lower')
    plt.axis('image')
    plt.axis('off')
    plt.clim(0,1)
    plt.plot(xr4,yr4,'w',linewidth=linew)
    ...
    if SaveImage is True:
        fu.SaveImagec(Name + '_HaftoneLiethicknessCMYK_' + Name2[lauf])
    plt.show()
